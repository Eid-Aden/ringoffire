...
export class GameCardComponent implements OnInit, OnChanges {
  cardAction = [
    ...
    {
      title: "Rule",
      description: "Sheeg Magaca Qofka ugu wayn Qoyskiina .",
    },
  ];
  ...
  ngOnChanges(): void {
    if (this.cardbox) {
      console.log("the Current Card is:", this.cardbox);

      let cardBoxNumber = +this.cardbox.split("_")[1];
      this.title = this.cardAction[cardBoxNumber - 1].title;
      this.description = this.cardAction[cardBoxNumber - 1].description;
    }
  }
}
